                                                            ...
                                                            unsigned                            i_layer)
{
  ...
  for (unsigned i_dmrs_symbol = 1; i_dmrs_symbol != nof_dmrs_symbols; ++i_dmrs_symbol) {
    ...
    std::string csi_rx_pilot_list = "";
    csi_rx_pilot_list += "[";
    unsigned int pp_index = 0;
    for (cf_t pp : pilot_products) {
      if (pp_index == pilot_products.size() - 1) {
        csi_rx_pilot_list += "(" + std::to_string(pp.real()) + "," + std::to_string(pp.imag()) + ")";
      } else {
        csi_rx_pilot_list += "(" + std::to_string(pp.real()) + "," + std::to_string(pp.imag()) + "),";
      }
      pp_index++;
    ...
    printf("[CSI_EQ]:%lld;%u;%u;%zu;%s\n",
          (long long)currentTime,
          i_dmrs_symbol,
          i_layer,
          pilot_products.size(),
          csi_rx_pilot_list.c_str());                  
